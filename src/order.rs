...
impl From<sys::Order> for Order {
    fn from(input: sys::Order) -> Order {
        unsafe {
            let id = input.id;
            assert!(id >= 0);
            assert!(id < Order::MAX as i32);
            ::std::mem::transmute(id)
        }
    }
}

impl From<Order> for sys::Order {
    fn from(input: Order) -> sys::Order {
        use std::os::raw::c_int;
        sys::Order { id: input as c_int }
    }
}
